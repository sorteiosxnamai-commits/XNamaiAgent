...
def test_extract_product_query_removes_commercial_prefixes():
    assert extract_product_query("Tem MarcaA ChargeMax?") == "MarcaA ChargeMax"
    assert extract_product_query("Vocês têm o T120.417.11.051.00?") == "T120.417.11.051.00"
    assert extract_product_query("Quanto custa o MarcaG?") == "MarcaG"
    assert extract_product_query("Tem estoque de EAN 7611608287637?") == "EAN 7611608287637"


def test_resolve_commerce_actions():
    assert resolve_commerce_action("Tem MarcaG?") == "product_search"
    assert resolve_commerce_action("Quanto custa o MarcaA?") == "product_price"
    assert resolve_commerce_action("Tem estoque do MarcaA?") == "product_inventory"
...
async def test_product_search_is_deterministic_and_does_not_need_openai(monkeypatch):
    ...
    async def fake_execute(name, arguments):
        ...
        return {"products": [{"id": "641", "name": "MarcaA ChargeMax", "reference": "T120.417.11.051.00", "current_price": 6399.99}]}

    monkeypatch.setattr("app.commerce_router.execute_tool", fake_execute)
    result = await handle_commerce_message(IncomingMessage(text="Tem MarcaA ChargeMax?"), {"primary_intent": "commerce"}, {})
    assert calls == [("search_products", {"query": "MarcaA ChargeMax", "limit": 3})]
...
async def test_agent_commerce_calls_tray_before_openai(monkeypatch):
    ...
    async def fake_execute(name, arguments):
        calls.append((name, arguments))
        return {"products": [{"id": "1", "name": "MarcaA ChargeMax", "current_price": 100}]}
    ...
    result = await openai_agent.generate_agent_reply_async(IncomingMessage(text="Tem MarcaA ChargeMax?"), {})
...
async def test_inventory_searches_then_checks_single_product(monkeypatch):
    ...
    async def fake_execute(name, arguments):
        ...
        if name == "search_products":
            return {"products": [{"id": "641", "name": "MarcaA ChargeMax"}]}
...
async def test_follow_up_inventory_refreshes_tray_using_identity_only(monkeypatch):
    ...
    async def fake_execute(name, arguments):
        calls.append((name, arguments))
        if name == "search_products":
            return {"products": [{"id": "641", "name": "MarcaA ChargeMax", "reference": "T120"}]}
        return {"product_id": "641", "stock": 38, "availability": "Disponível"}

    monkeypatch.setattr("app.commerce_router.execute_tool", fake_execute)
    context = {"conversation_id": "conversation-1"}
    await handle_commerce_message(IncomingMessage(conversation_id="conversation-1", text="Tem MarcaA ChargeMax?"), {"primary_intent": "commerce"}, context)
    ...
    assert calls == [
        ("search_products", {"query": "MarcaA ChargeMax", "limit": 3}),
        ("check_inventory", {"product_id": "641"}),
    ]
...
async def test_follow_up_price_refreshes_product_and_does_not_use_old_price(monkeypatch):
    ...
    async def fake_execute(name, arguments):
        calls.append((name, arguments))
        if name == "search_products":
            return {"products": [{"id": "641", "name": "MarcaA ChargeMax", "current_price": 100}]}
        return {"id": "641", "name": "MarcaA ChargeMax", "current_price": 200, "payment_option_details": "Pix"}

    monkeypatch.setattr("app.commerce_router.execute_tool", fake_execute)
    await handle_commerce_message(IncomingMessage(sender_phone="5511999999999", text="Tem MarcaA ChargeMax?"), {"primary_intent": "commerce"}, {})
...
async def test_follow_up_tray_error_does_not_reuse_previous_dynamic_data(monkeypatch):
    ...
    async def fake_execute(name, arguments):
        calls.append((name, arguments))
        if name == "search_products":
            return {"products": [{"id": "641", "name": "MarcaA ChargeMax", "current_price": 100, "stock": 38}]}
        return {"error": "unavailable"}

    monkeypatch.setattr("app.commerce_router.execute_tool", fake_execute)
    await handle_commerce_message(IncomingMessage(sender_phone="5511888888888", text="Tem MarcaA ChargeMax?"), {"primary_intent": "commerce"}, {})
...
async def test_not_found_is_not_technical_error_and_tray_error_is_neutral(monkeypatch):
    ...
    failed = await handle_commerce_message(IncomingMessage(text="Tem MarcaA?"), {"primary_intent": "commerce"}, {})
